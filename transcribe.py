...
tts_authenticator = IAMAuthenticator('J1j6shP-5GZ7sUYFZZLUiqipN4KY_sXVqryrXb1u3Ls-')
tts = TextToSpeechV1(authenticator=tts_authenticator)
tts.set_service_url('https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/ee776c76-341f-445e-98d8-0e5be18640c0')



authenticator = IAMAuthenticator('w6NF-PFopm_-d8UVBSjk3sbuXHzgmnVGKkNC_9xmxtnY')
...
response = assistant.create_session(
    assistant_id='2854db5f-fcaa-4eef-99ad-e286adfb974b'
        ).get_result()
...
def read_audio(ws, timeout):
    ...
    txtfile = open("userOutput.txt","r+", encoding='utf-8')
    ...
    try:
        ...
        savevoice(resp['output']['generic'][0]['text'])
    except:
        resp = u"لم افهم ما تقصدو الرجاء اعادة المحاولة"
        print(resp[::-1])
        savevoice(resp)
    
        
def savevoice(v):
    with open('bot.mp3', 'wb') as audfile:
        res = tts.synthesize(v, accept='audio/mp3', voice='ar-MS_OmarVoice').get_result()
        audfile.write(res.content)
    playsound('./bot.mp3')
...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        txtfile = open("userOutput.txt","w",encoding='utf-8')
        txtfile.write(data['results'][0]['alternatives'][0]['transcript'])
        txtfile.close()
